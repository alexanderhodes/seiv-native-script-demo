...
export class FileService {
    ...
    readWithKey(key: native.Array<number>, path: string): string {
        ...
        const data = new java.lang.String(decodedData, java.nio.charset.Charset.forName('UTF-8'));
        return data ? data.toString() : null;
    }

    saveWithKey(key: native.Array<number>, text: string, path: string): void {
        const bytes = new java.lang.String(text).getBytes(java.nio.charset.Charset.forName('UTF-8'));
